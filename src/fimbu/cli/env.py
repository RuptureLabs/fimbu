...
class MigrationEnv:
    ...
    def load_from_env(
        self, app_path: typing.Optional[str] = None, enable_logging: bool = True
    ) -> "MigrationEnv":
        """
        ...
        _path = os.getenv(EDGY_DISCOVER_APP) if not app_path else app_path
        _app = self.find_app(path=_path, cwd=cwd)

        return MigrationEnv(app_path=_app.path, app=_app.app)
...
class FimbuEnv(LitestarEnv, MigrationEnv):...